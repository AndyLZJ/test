...
//******************用户-用户组*******************
    ...
    public function check_user(){
        ...
        $user_ids=D('AuthGroupAccess')->getuser_idsByGroupId($group_id);
        // 判断用户名是否为空
        if(empty($username)){
            $user_data='';
        }else{
    ...
    public function edit_admin(){
        if(IS_POST){
            ...
            // 修改权限
            D('AuthGroupAccess')->deleteData(array('user_id'=>$user_id));
            foreach ($data['group_ids'] as $k => $v) {
                $group=array(
                    'user_id'=>$user_id,
                    'group_id'=>$v
            ...
            if($result){
                // 操作成功
                $this->success('编辑成功',U('Admin/Rule/edit_admin',array('id'=>$user_id)));
            }else{
                $error_word=D('Users')->getError();
                if (empty($error_word)) {
                    $this->success('编辑成功',U('Admin/Rule/edit_admin',array('id'=>$user_id)));

...
class ManageController extends CommonController{
	...
	public function saveCourse(){
        //判断用户是否存在,获取用户id,判断提交方式是否合法
        $userId = $this->verifyUserDataPost();
        $post["project_id"] = I("post.project_id",0,'int');
        $post["course_id"] = I("post.course_id",0,'int');
        ...
        $post["eidt_id"] = I("post.eidt_id",0,'int');
		...
		if(empty($post["course_id"])){
                $this->error(1030, "请选择课程");
        }

		if($post["lecturer_id"] < 1){
			$this->error(1030, "参数有误： lecturer_id 讲师id");
		}
		...
		$timeVerifi = self::timeVerifi($post["start_time"], $post["end_time"]);
		if($timeVerifi["code"] != 1000){
			$this->error(1030, $timeVerifi["message"]);
		}
		...
		$return = D("Manage")->saveCourse($post,$userId,$type);
		if($return["code"] == 1000){
			$this->success(1000, $return["message"], $return["data"]);
		}else{
			$this->error($return["code"], $return["message"]);

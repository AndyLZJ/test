...
class PublicCourseModel extends CommonModel{
     ...
     * 首页公开课程列表
     */
    public function homePage($page,$total,$keyword,$userId){
        ...
        $data = M('Course')
              ->field("id,course_name,course_cover,FROM_UNIXTIME(create_time,'%Y-%m-%d %H:%i:%s') create_time")
              ->where($where)
    ...
    public function homePageCourse($type,$userId,$page,$pageLen){
        ...
        if($type == 1){
            ...
            $results = M("course as a")
                ->join("JOIN __PROJECT_COURSE__ as b ON a.id=b.course_id")
                ->join("JOIN __DESIGNATED_PERSONNEL__ as c ON b.project_id = c.project_id")
                ->join("JOIN __ADMIN_PROJECT__ as d ON b.project_id = d.id")
                ->join("JOIN __COURSE_DETAIL__ as e ON a.id = e.id")
                ->field("b.project_id,a.id as course_id,a.course_name,a.course_cover,d.start_time,d.end_time,e.course_intro")->where($where)->select();
            ...
            $data = M("Course as a")
                ->join("LEFT JOIN __COURSE_DETAIL__ as b ON a.id = b.id")
                ->join("JOIN __COURSE_RECORD__ as c ON a.id = c.course_id")
                ->field("a.id as course_id,a.course_name,a.course_cover,b.course_intro,c.project_id")->where($condition)->select();
            $k = 0;
            foreach ($data as $val){
            ...
            if(!empty($info) || !empty($res)){
                $array_data = array_merge_recursive($info,$res);
            }
            $sort = array(
           ...
           'field'     => 'course_id',       //排序字段
            ...
            foreach ($data as $val){
                ...
                $_chapterWhere["project_id"] = 0;
                $_isStudyChapter = M("Course_chapter")->field("id")->where($_chapterWhere)->count();
                if($_isStudyChapter == count($_chapter)){
                    //已完成
                    $res[$k]['_status'] = 3;
                    $res[$k]['status'] = '已结束';
                }

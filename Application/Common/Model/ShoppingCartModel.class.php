...
class ShoppingCartModel extends BaseModel{
     ...
     */
    public function addData($user_id,$goods_id,$number=1){
        // 获取产品数据
        ...
        if (empty($data)) {
            ...
            // 已存在、增加数量
            $number +=$data['number'];
            $this->setNumberById($user_id,$goods_id,$number);
        }
        // 添加成功
        $result=array(
            'error_code'=>0,
     ...
     * @param integer $number   数量
     */
    public function setNumberById($user_id,$goods_id,$number){
        $map=array(
            'user_id'=>$user_id,
            'goods_id'=>$goods_id
            );
    ...
    public function getCountData($user_id,$goods_ids){

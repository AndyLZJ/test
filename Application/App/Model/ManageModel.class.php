...
class ManageModel extends CommonModel{
	 ...
	 * 
	 */
	public function getCourse($param,$uid){
		if(!$param || !$uid){
			return array("code"=>1021, "message"=>'提交失败，未获取到参数或用户数据');
		}
		...
		$course = M("course")->field("id as course_id,course_name,course_code,course_time,create_time,lecturer as lecturer_id")->where($where)->select();
		foreach ($course as $key=>$value){
			$course[$key]["create_time"] = date("Y-m-d H:i:s", $course[$key]["create_time"]);
			
			$lector = M("lecturer")->field("name")->where("id=".$value["lecturer_id"])->limit(1)->select();
			$course[$key]["lecturer_name"] = $lector[0]["name"];
		}
		return array("code"=>1000, "message"=>'操作成功', "data"=>$course);
	}
	 ...
	 * 添加课程-保存课程
	 * project_id 项目id
	 * course_id 课程id (不一定会填写，手动输入课程名称的情况下)
	 * course_name 课程名称
	 * lecturer_id 讲师id
	 * start_time 开始时间
	 * end_time 结束时间
	 ...
	 * edit_id 修改课程id 修改时需传入
	 */
	public function saveCourse($param,$uid){
		if(!$param || !$uid){
			return array("code"=>1021, "message"=>'提交失败，未获取到参数或用户数据');
		}
		...
		if(isset($param["edit_id"]) && $param["edit_id"] > 0){
			...
			$repeatWhere["course_names"] = $param["course_name"];
			$repeatWhere["_string"] = " id!=".$param["edit_id"];
           ...
           //循环拼接指定参与人
			...
			foreach ($list as $key=>$value){
                ...
                if($value['course_id'] != '' || $value['course_id'] != null){
                    $course = M("course")->field("course_name")->where("id=".$list[$key]["course_id"])->limit(1)->select();
                    $list[$key]["course_names"] = $course[0]["course_name"];
                }
				$lecturer = M("lecturer")->field("name")->where("id=".$value["lecturer_id"])->limit(1)->select();
				$list[$key]["lecturer_name"] = $lecturer[0]["name"];//讲师名称
			}

...
class MineModel extends CommonModel {
    ...
    public function mine($userId){
        ...
        $userStudyTime = M('Center_study')->where(array('user_id'=>$userId))->sum('hours');
        $userStudyTime = $userStudyTime ? $userStudyTime : 0;
        //把学时分钟转化为小时
        $userInfo['userStudyTime'] = floor(($userStudyTime/60))."小时";
     ...
     * $course_id 课程id
     */
     public function doComment($post,$uid){
         ...
         if(!empty($post['course_id']) && !empty($post['comment_content'])){
             ...
             //$res = M('ColligateComment')->where(array('uid'=>$uid,'course_id'=>$post['course_id'],'lecturer_id'=>$lecturer_id))->find();
                //课程评价
                 $data['uid'] = $post['uid'];
                 $data['course_id'] =  $post['course_id'];
                 $data['lecturer_id'] =  $post['lecturer_id'];
             ...
             if($result){
                  return $this->error(1030,'评分只能评一次');
             }else{
                 $infop = M('Course_score')->data($array)->add();
             }
             if($info){
                 return $this->success(1000,'操作成功',array('id'=>$info,'uid'=>$uid,'course_id'=>$post['course_id']));
             }else{
                 return $this->error(1030,'操作失败');
             }
             }else{
             return $this->error(1013,'缺少必要参数');
         }
     }

    /*else{
            $post['comment_time'] = date('Y-m-d H:i:s',time());
                 $info = M('ColligateComment')->where(array('uid'=>$uid,'course_id'=>$post['course_id'],'lecturer_id'=>$lecturer_id))->save($post);
                 if($info){
                     return $this->success(1000,'操作成功',array('id'=>$info,'uid'=>$uid,'course_id'=>$post['course_id']));
                 }else{

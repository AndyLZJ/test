  ...
  <link rel="stylesheet" href="/Public/plugins/jvectormap/jquery-jvectormap-1.2.2.css">
	...
	<body>
		...
		<ul id="pn">
			<!--	初始文本框区域-->
			<!--	初始文本框区域结束-->
            <?php if(is_array($list)): foreach($list as $key=>$v): ?><li class="list0">
				<?php if($v['user_id'] == $mytagid): ?><a  class="close" onclick="dele_btn(<?php echo ($v["id"]); ?>,1)" href="javascript:;">X</a><?php endif; ?>
				<div class="head"><img src="<?php echo ($v["avatar"]); ?>"  onerror="this.src='/Upload/avatar/default.png'" /></div>
				<div id="c<?php echo ($v["id"]); ?>" class="HF_content">
					<p class="text"><span class="name"><?php if($mytagid == $v['user_id']): ?>我<?php else: echo ($v["username"]); endif; ?>：</span><?php echo ($v["content"]); ?></p>
					<div class="good"><span class="date"><?php echo ($v["publish_time"]); ?></span>
		                				
					  <?php if($v["whetherPraise"] == 1): ?><a class="dzan" onclick="DZ_btn(<?php echo ($v["id"]); ?>)" href="javascript:;">取消赞</a>
					
					  <?php else: ?>
                        <a class="dzan" onclick="DZ_btn(<?php echo ($v["id"]); ?>)" href="javascript:;">赞</a><?php endif; ?>
                      
						
						<a href="javascript:void(0);" style="color: red;float:right; margin-right: 50px;"><span class="shousuo" value="1" id="ggwz_1">收起回复▲</span>(<span id="commentnum"><?php echo ($v["subComments"]); ?></span>)</a>
					</div>
					<?php if($v["praise"] == ''): ?><div class="people" total="0">0人觉得很赞</div> 
                    <?php else: ?>
					<div class="people" total="<?php echo ($v["praise"]); ?>"><?php echo ($v["praise"]); ?>人觉得很赞</div><?php endif; ?>
					<!--子评论-->
					<div id="shou_suo_1" class="comment-list">
						<?php if(!empty($v['subReply'])): if(is_array($v['subReply'])): foreach($v['subReply'] as $key=>$v1): ?><div class="comment" user="self">
							<div class="comment-left"><img onerror="this.src='/Upload/avatar/default.png'" src="<?php echo ($v1["avatar"]); ?>" alt="" /></div>
							<div class="comment-right">
                                
								<!--<empty name="v1.toname">-->
									<?php if($v1['_level'] == 2): ?><div id="c<?php echo ($v1["id"]); ?>" class="comment-text"><span class="user">
									<?php if($mytagid == $v1['user_id']): ?>我<?php else: echo ($v1["username"]); endif; ?>：</span><?php echo ($v1["content"]); ?>
									</div>
								<?php else: ?> 
								<div id="c<?php echo ($v1["id"]); ?>" class="comment-text"><span class="user"><?php if($mytagid == $v1['user_id']): ?>我<?php else: echo ($v1["username"]); endif; ?> </span><span class="tonamecs">&nbsp;回复<?php if($mytagid == $v1['touser_id']): ?>我<?php else: echo ($v1["toname"]); endif; ?> ：</span><?php echo ($v1["content"]); ?></div><?php endif; ?>

								<!--<div class="comment-text"><span class="user"><?php echo ($v1["username"]); ?></span><?php echo ($v1["content"]); ?></div>-->
								<div class="comment-date"><?php echo ($v1["publish_time"]); ?>
									<a style="right:45px;" class="comment-dele" href="javascript:;" onclick="HF_btn(<?php echo ($v1["id"]); ?>,<?php echo ($v1["user_id"]); ?>)">回复</a>
									<?php if($v1['user_id'] == $mytagid): ?><a class="comment-dele" href="javascript:;" onclick="dele_btn(<?php echo ($v1["id"]); ?>)">删除</a><?php endif; ?>
								</div>
							</div>
					...
					<div class="hf">
						<textarea type="text" class="hf-text" autocomplete="off" maxlength="100">请输入您想要说的…</textarea>
						<button class="hf-btn" onclick="HF_btn1(<?php echo ($v["id"]); ?>,<?php echo ($v["user_id"]); ?>)">提交</button>
					</div>
				</div>
		...
		<script src="/Public/Dist/js/utf8-php/ueditor.config.js"></script>
		<script src="/Public/Dist/js/utf8-php/ueditor.all.js"></script>
		<script src="/Public/Dist/js/utf8-php/lang/zh-cn/zh-cn.js"></script>
		...
		<script type="text/javascript">
					...
					function dele_btn(d,t)
	             	{
					  ...
					  var e = getEvent();
						...
						if(type == 1){  
								$.post('<?php echo U('Admin/FriendsCircle/delFriendsCircle');?>',{id:d}, function($data) {  
			...
			function b() {
				...
				function praisebox(box, el) {
					...
					//判断点击的文字内容
					if(txt == "赞") {
						$.post('<?php echo U('Admin/FriendsCircle/praiseFriendsCircle');?>',{id:cid,type:'praise'}, function($data) {  
							if($data){  
						...
						$.post('<?php echo U('Admin/FriendsCircle/praiseFriendsCircle');?>',{id:cid,type:'cancel'}, function($data) {  
							if($data){  
				...
				function reply(box) {
					...
					//获取回复当前人 的用户头像
					var avatar = "<?php echo ($mytagavatar); ?>";
                    ...
                    // alert($(comment).parents('.HF_content').children('.good').find("#commentnum").html());exit;
					$.post('<?php echo U('Admin/FriendsCircle/replyFriendsCircle');?>',{content:textareavalue,type:'reply',pid:cid}, function($data) {  
                          ...
                          if($data){  
							//   回复评论成功后改评论数
							  ...
							  layer.msg('回复成功',{time: 1000,icon: 1});//bootstrap框架确认弹窗
							  cid = $data;
				...
				for(var i = 0; i < lists.length; i++) {
					...
					textarea.onfocus = function() {
							...
							this.value = this.value == '请输入您想要说的…' ? '' : this.value;
						}
						//失焦事件
...
//						textNub.innerHTML = len + "/100";
			...
			//评论收缩 废弃
			function ShouSuo(Id) {
				//判断Id等于评论Id (注意评论Id是活的需要从后台传值)
				//展示与收起回复中的Id后面的数字就是你评论Id
			...
			function FaBu() {
				...
				// alert(html);
                $.post("<?php echo U('Admin/FriendsCircle/publicFriendsCircle');?>",{content:html,type:'public'}, function($data) {  
                          ...
                          if($data.status == 1){  
							  function clearmyedit(){
								...
								clearmyedit();
							  layer.msg('话题发布成功，请等待审核结果!',{time: 2000,icon: 1});//bootstrap框架确认弹窗
				...
				// var html = ue.getContent();
				// var Centont = html; //发布内容
				// var url = "../dist/img/avatar5.png"; //图片路径
				// var html = "<li class=\"list0\"><a class=\"close\" href=\"javascript:;\">X</a><div class=\"head\"><img src=\"" + url + "\"/></div><div class=\"HF_content\"><p class=\"text\"><span class=\"name\">" + UserName + "：</span>" + Centont + "</p><div class=\"pic\"></div><div class=\"good\"><span class=\"date\">02-14 23:01</span><a class=\"dzan\" href=\"javascript:;\">赞</a><a href=\"javascript:void(0);\" onclick=\"ShouSuo(2)\" style=\"color: red;float:right; margin-right: 50px;\"><span id=\"ggwz_2\">收起回复▲</span></a></div><div class=\"people\" total=\"0\">0人觉得很赞</div><div id=\"shou_suo_2\" class=\"comment-list\"></div><div class=\"hf\"><textarea type=\"text\" class=\"hf-text\" autocomplete=\"off\" >请输入您想要说的…</textarea><button class=\"hf-btn\">回复</button></div></div></li>";
				// $("#pn").prepend(html);
				// b();

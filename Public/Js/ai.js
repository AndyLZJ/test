...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    str = "<div class=process-status><strong>上传状态:</strong><span> " + '已完成' + "</span></div>";
    ...
    //var processedPlayBtn = $('<input type="hidden" name="enclosure[]" value="'+url+'"/>');
    //var processedPlayBtns = $('<a class="btn  btn-info play-btn" href="'+url+'" target="_blank">浏览附件</a>');

    //Wrapper.append(playBtn);
    //Wrapper.append(processedPlayBtn);
    //Wrapper.append(processedPlayBtns);
    //progressNameTd.append(Wrapper);
    $('table button.btn').hide().parents('tr').next().hide(); //隐藏进度按钮

    $("#media_address").val(url);
    var processedLink = up.getOption('domain');
    //var callback_url = $("#callback_url").val();
    /*
    timerId = setInterval(function() {
        statusUrl = 'pfopStatus';
        statusAnchor = td.find('.process-status span');

        $.ajax({
            url: callback_url,
            async: false,
            data: "id="+res.persistentId,
        }).done(function(resp) {
            ...
            switch (item.code) {
                case 0:
                    ...
                    processedPlayBtns.show();
                    processedLink += encodeURIComponent(item.key);
                    //$("#media_address").val(processedLink);
                    processedPlayBtn.attr('value', processedLink);
                    processedPlayBtns.attr('href',processedLink);
